...
public class SchematicListWidget extends ObjectSelectionList<SchematicListWidget.BaseEntry> {
    ...
    public static class ClipboardEntryWidget extends BaseEntry {
        private final com.schematicraft.bg2addon.core.ClipboardEntry clip;
        private final java.util.function.BiConsumer<String, String> onLoad;
        private final java.util.function.Consumer<com.schematicraft.bg2addon.core.ClipboardEntry> onSave;
        private java.util.function.Consumer<com.schematicraft.bg2addon.core.ClipboardEntry> onHover;

        public ClipboardEntryWidget(SchematicListWidget parent,
                                     com.schematicraft.bg2addon.core.ClipboardEntry clip,
                                     java.util.function.BiConsumer<String, String> onLoad,
                                     java.util.function.Consumer<com.schematicraft.bg2addon.core.ClipboardEntry> onSave) {
            super(parent);
            this.clip = clip;
            this.onLoad = onLoad;
            this.onSave = onSave;
        ...
        public com.schematicraft.bg2addon.core.ClipboardEntry getClipboardEntry() { return clip; }

        public void setOnHover(java.util.function.Consumer<com.schematicraft.bg2addon.core.ClipboardEntry> onHover) {
        ...
        public void render(GuiGraphics graphics, int index, int top, int left,
            ...
            String title = clip.getDisplayName();
            int maxW = width - 24;
            if (mc.font.width(title) > maxW) {
                while (mc.font.width(title + "...") > maxW && title.length() > 3)
                    title = title.substring(0, title.length() - 1);
                title += "...";
            }
            graphics.drawString(mc.font, title, left + 3, top + 2, hovered ? 0xFFFF55 : 0xE0E0E0);

            String subtitle = "\u00a78" + clip.getTimeAgo();
            if (clip.hasImages()) subtitle += " \u00a7b" + clip.getImageCount() + " img";
            graphics.drawString(mc.font, subtitle, left + 3, top + 13, 0x606060);

            if (!clip.isUploaded()) {
                int saveX = left + width - 18;
                boolean saveHovered = mouseX >= saveX && mouseX <= saveX + 16
                        && mouseY >= top && mouseY <= top + height;
                graphics.drawString(mc.font, "\u2B06", saveX, top + 5,
                        saveHovered ? 0x55FF55 : 0x8888CC);
            } else {
                graphics.drawString(mc.font, "\u2713", left + width - 14, top + 5, 0x55FF55);
            }

            if (hovered) {
                graphics.fill(left, top, left + width - 20, top + height, 0x1800FF00);
                graphics.fill(left, top, left + 1, top + height, 0x4000FF00);
                if (onHover != null) onHover.accept(clip);
            }
        }
        ...
        public boolean mouseClicked(double mouseX, double mouseY, int button) {
            ...
            int left = parent.listLeft;
            int width = parent.getRowWidth();
            int saveX = left + width - 20;
            if (mouseX >= saveX && !clip.isUploaded()) {
                onSave.accept(clip);
                return true;
            }

            onLoad.accept(clip.getGadgetUuid().toString(), clip.getDisplayName());
            return true;
        }

        @Override
        public Component getNarration() {
            return Component.literal(clip.getDisplayName());
        }
    }
}
